...
def net(
    ...
    def get_dlas(input_shape, device):
        dlas0 = []
        for backbone_name, layers_to_extract_from in zip(backbone_names, layers_to_extract_from_coll):
            ...
            dlas0.append(dlas_inst.to(device))
        return dlas0

    return "get_dlas", get_dlas
...
def dataset(
    ...
    _DATASETS = {"mvtec": ["datasets.mvtec", "MVTecDataset"], "visa": ["datasets.visa", "VisADataset"],
                 ...
                 "anolcd": ["datasets.anolcd", "MVTecDataset"],}
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        dlas_list = methods["get_dlas"](imagesize, device)
        ...
        for i, DLAS in enumerate(dlas_list):
            flag = 0., 0., 0., 0., 0., -1.
            if DLAS.backbone.seed is not None:
                utils.fix_seeds(DLAS.backbone.seed, device)

            DLAS.set_model_dir(os.path.join(models_dir, f"backbone_{i}"), dataset_name)




            if test == 'ckpt':
                flag = DLAS.trainer(dataloaders["training"], dataloaders["testing"], dataset_name)
                if type(flag) == int:
            ...
            if type(flag) != int:
                i_auroc, i_ap, p_auroc, p_ap, p_pro, epoch = DLAS.tester(dataloaders["testing"], dataset_name)
                # ---- Manifold & neighborhood validation (extra outputs, no logic change)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.INFO, encoding="utf-8")  # ← 加上 encoding
